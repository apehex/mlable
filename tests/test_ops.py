...
class MaskingTest(tf.test.TestCase):
    ...
    def test_reduce_shapes(self):
        ...
        __m_0_any = mlable.ops._reduce_any(tensor=self._ascii, axis=0, keepdims=False)
        __m_0_all = mlable.ops._reduce_all(tensor=self._ascii, axis=0, keepdims=False)
        __m_1_any = mlable.ops._reduce_any(tensor=self._ascii, axis=-1, keepdims=False)
        __m_1_all = mlable.ops._reduce_all(tensor=self._ascii, axis=-1, keepdims=False)
        ...
        # reduced on all axes
        __m_any = mlable.ops._reduce_any(tensor=self._ascii, axis=None, keepdims=False)
        __m_all = mlable.ops._reduce_all(tensor=self._ascii, axis=None, keepdims=False)
        self.assertEqual(list(__m_any.shape), [])
        self.assertEqual(list(__m_all.shape), [])
        # same shape as input
        __m_0_any_keep = mlable.ops._reduce_any(tensor=self._ascii, axis=0, keepdims=True)
        __m_0_all_keep = mlable.ops._reduce_all(tensor=self._ascii, axis=0, keepdims=True)
        __m_1_any_keep = mlable.ops._reduce_any(tensor=self._ascii, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.ops._reduce_all(tensor=self._ascii, axis=-1, keepdims=True)
        __m_any_keep = mlable.ops._reduce_any(tensor=self._ascii, axis=None, keepdims=True)
        __m_all_keep = mlable.ops._reduce_all(tensor=self._ascii, axis=None, keepdims=True)
    ...
    def test_group_shapes(self):
        # reduced along a single axis
        __m_0_any = mlable.ops._reduce_group_by_group_any(tensor=self._ascii, group=2, axis=0, keepdims=False)
        __m_0_all = mlable.ops._reduce_group_by_group_all(tensor=self._ascii, group=2, axis=0, keepdims=False)
        __m_1_any = mlable.ops._reduce_group_by_group_any(tensor=self._ascii, group=4, axis=-1, keepdims=False)
        __m_1_all = mlable.ops._reduce_group_by_group_all(tensor=self._ascii, group=4, axis=-1, keepdims=False)
        ...
        # same shape as input
        __m_0_any_keep = mlable.ops._reduce_group_by_group_any(tensor=self._ascii, group=2, axis=0, keepdims=True)
        __m_0_all_keep = mlable.ops._reduce_group_by_group_all(tensor=self._ascii, group=2, axis=0, keepdims=True)
        __m_1_any_keep = mlable.ops._reduce_group_by_group_any(tensor=self._ascii, group=4, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.ops._reduce_group_by_group_all(tensor=self._ascii, group=4, axis=-1, keepdims=True)
        self.assertEqual(list(__m_0_any_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_0_all_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_1_any_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_1_all_keep.shape), list(self._ascii.shape))
    ...
    def test_group_on_padded_values(self):
        # reduce 4 by 4
        __m_1_any = mlable.ops.reduce_any(tensor=self._padded, group=4, axis=-1, keepdims=False)
        __m_1_all = mlable.ops.reduce_all(tensor=self._padded, group=4, axis=-1, keepdims=False)
        self.assertAllEqual(__m_1_any, tf.convert_to_tensor([14 * [True] + [False, False], 4 * [False] + 10 * [True] + [False, True]], dtype=tf.dtypes.bool))
        self.assertAllEqual(__m_1_all, tf.zeros(shape=(2, 16), dtype=tf.dtypes.bool))
        # same shape as input
        __m_1_any_keep = mlable.ops.reduce_any(tensor=self._padded, group=4, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.ops.reduce_all(tensor=self._padded, group=4, axis=-1, keepdims=True)

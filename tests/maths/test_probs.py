...
class LogNormalDistributionTest(tf.test.TestCase):
    ...
    def test_shape_and_dtype(self):
        for __case in self._cases:
            __outputs = mlable.maths.probs.log_normal_pdf(**__case)
    ...
    def test_specific_values(self):
        for __case in self._cases:
            __outputs = mlable.maths.probs.log_normal_pdf(sample=__case['mean'], mean=__case['mean'], logvar=__case['logvar'])

...
class MaskingTest(tf.test.TestCase):
    ...
    def test_reduce_shapes(self):
        ...
        __m_0_any = mlable.maths.ops._reduce_any(data=self._ascii, axis=0, keepdims=False)
        __m_0_all = mlable.maths.ops._reduce_all(data=self._ascii, axis=0, keepdims=False)
        __m_1_any = mlable.maths.ops._reduce_any(data=self._ascii, axis=-1, keepdims=False)
        __m_1_all = mlable.maths.ops._reduce_all(data=self._ascii, axis=-1, keepdims=False)
        ...
        # reduced on all axes
        __m_any = mlable.maths.ops._reduce_any(data=self._ascii, axis=None, keepdims=False)
        __m_all = mlable.maths.ops._reduce_all(data=self._ascii, axis=None, keepdims=False)
        self.assertEqual(list(__m_any.shape), [])
        self.assertEqual(list(__m_all.shape), [])
        # same shape as input
        __m_0_any_keep = mlable.maths.ops._reduce_any(data=self._ascii, axis=0, keepdims=True)
        __m_0_all_keep = mlable.maths.ops._reduce_all(data=self._ascii, axis=0, keepdims=True)
        __m_1_any_keep = mlable.maths.ops._reduce_any(data=self._ascii, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.maths.ops._reduce_all(data=self._ascii, axis=-1, keepdims=True)
        __m_any_keep = mlable.maths.ops._reduce_any(data=self._ascii, axis=None, keepdims=True)
        __m_all_keep = mlable.maths.ops._reduce_all(data=self._ascii, axis=None, keepdims=True)
    ...
    def test_group_shapes(self):
        # reduced along a single axis
        __m_0_any = mlable.maths.ops._reduce_group_by_group_any(data=self._ascii, group=2, axis=0, keepdims=False)
        __m_0_all = mlable.maths.ops._reduce_group_by_group_all(data=self._ascii, group=2, axis=0, keepdims=False)
        __m_1_any = mlable.maths.ops._reduce_group_by_group_any(data=self._ascii, group=4, axis=-1, keepdims=False)
        __m_1_all = mlable.maths.ops._reduce_group_by_group_all(data=self._ascii, group=4, axis=-1, keepdims=False)
        ...
        # same shape as input
        __m_0_any_keep = mlable.maths.ops._reduce_group_by_group_any(data=self._ascii, group=2, axis=0, keepdims=True)
        __m_0_all_keep = mlable.maths.ops._reduce_group_by_group_all(data=self._ascii, group=2, axis=0, keepdims=True)
        __m_1_any_keep = mlable.maths.ops._reduce_group_by_group_any(data=self._ascii, group=4, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.maths.ops._reduce_group_by_group_all(data=self._ascii, group=4, axis=-1, keepdims=True)
        self.assertEqual(list(__m_0_any_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_0_all_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_1_any_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_1_all_keep.shape), list(self._ascii.shape))
    ...
    def test_group_on_padded_values(self):
        # reduce 4 by 4
        __m_1_any = mlable.maths.ops.reduce_any(data=self._padded, group=4, axis=-1, keepdims=False)
        __m_1_all = mlable.maths.ops.reduce_all(data=self._padded, group=4, axis=-1, keepdims=False)
        self.assertAllEqual(__m_1_any, tf.convert_to_tensor([14 * [True] + [False, False], 4 * [False] + 10 * [True] + [False, True]], dtype=tf.dtypes.bool))
        self.assertAllEqual(__m_1_all, tf.zeros(shape=(2, 16), dtype=tf.dtypes.bool))
        # same shape as input
        __m_1_any_keep = mlable.maths.ops.reduce_any(data=self._padded, group=4, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.maths.ops.reduce_all(data=self._padded, group=4, axis=-1, keepdims=True)
...
class BaseCompositionTest(tf.test.TestCase):
    ...
    def test_bidimensional_compositions(self):
        __number_10 = mlable.maths.ops.reduce_base(self._digits_10, base=10, axis=-1, bigendian=False)
        __number_2 = mlable.maths.ops.reduce_base(self._digits_2, base=2, axis=-1, bigendian=True)
        __number_256 = mlable.maths.ops.reduce_base(self._bytes, base=256, axis=-1, bigendian=True)
    ...
    def test_flat_compositions(self):
        __number_10 = mlable.maths.ops.reduce_base(self._digits_10_flat, base=10, axis=-1, bigendian=False)
        __number_2 = mlable.maths.ops.reduce_base(self._digits_2_flat, base=2, axis=-1, bigendian=True)
    ...
    def test_grouped_compositions(self):
        __number_10 = mlable.maths.ops.reduce_base(self._digits_10_flat, base=10, axis=-1, group=3, bigendian=False)
        __number_2 = mlable.maths.ops.reduce_base(self._digits_2_flat, base=2, axis=-1, group=3, bigendian=True)
        __number_256 = mlable.maths.ops.reduce_base(self._bytes_flat, base=256, axis=-1, group=2, bigendian=True)
        self.assertAllEqual(__number_10, tf.constant([321, 654, 987], dtype=tf.int32))
    ...
    def test_compositions_along_axis_0(self):
        __number_10 = mlable.maths.ops.reduce_base(self._digits_10, base=10, axis=0, bigendian=True)
        __number_2 = mlable.maths.ops.reduce_base(self._digits_2, base=2, axis=0, bigendian=True)
        __number_256 = mlable.maths.ops.reduce_base(self._bytes, base=256, axis=0, bigendian=True)
...
class BaseDecompositionTest(tf.test.TestCase):
    ...
    def test_decomposition_values(self):
        __number_10 = mlable.maths.ops.expand_base(self._digits_10, base=10, depth=3, bigendian=False)
        __number_2 = mlable.maths.ops.expand_base(self._digits_2, base=2, depth=4, bigendian=True)
        __number_256 = mlable.maths.ops.expand_base(self._bytes, base=256, depth=2, bigendian=True)

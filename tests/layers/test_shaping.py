...
class ReshapingTest(tf.test.TestCase):
    ...
    def test_reciprocity(self):
        for __t in self._tensors:
            __d = mlable.layers.shaping.Divide(input_axis=-2, output_axis=-1, factor=list(__t.shape)[-1], insert=True)
            __m = mlable.layers.shaping.Merge(left_axis=-2, right_axis=-1, left=True)
    ...
    def test_values_are_unchanged(self):
        for __t in self._tensors:
            __d = mlable.layers.shaping.Divide(input_axis=-2, output_axis=-1, factor=4, insert=True)
            __m = mlable.layers.shaping.Merge(left_axis=-2, right_axis=-1, left=True)
            if None not in list(__t.shape):

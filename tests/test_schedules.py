...
class CosineRatesTest(tf.test.TestCase):
        ...
        self._cases = [
            {
                'args': {
                    ...
                    'end_rate': 0.1,
                    'dtype': None,},
                'outputs': (tf.cast(0.9, tf.float32), tf.cast(math.sqrt(0.19), tf.float32)),},
            {
                'args': {
                    'angle_rates': 1.0,
                    'start_rate': 0.9,
                    'end_rate': 0.1,
                    'dtype': tf.float16,},
                'outputs': (tf.cast(0.1, tf.float16), tf.cast(math.sqrt(0.99), tf.float16)),},
            {
                'args': {
                    ...
                    'end_rate': 0.2,
                    'dtype': tf.float32,},
                'outputs': (tf.cast(4 * [0.2], tf.float32), tf.cast(4 * [math.sqrt(0.96)], tf.float32)),},
            {
                'args': {
                    ...
                    'end_rate': 0.0,
                    'dtype': None,},
                'outputs': (tf.cast([math.cos(0.25 * math.pi), math.cos(0.4 * math.pi)], tf.float32), tf.cast([math.sin(0.25 * math.pi), math.sin(0.4 * math.pi)], tf.float32)),},]

    def test_shape_and_dtype(self):
        for __case in self._cases:
            __cos_c, __sin_c = mlable.schedules.cosine_rates(**__case['args'])
            __cos_e, __sin_e = __case['outputs']
            # shapes
    ...
    def test_powers_sum_to_ones(self):
        for __case in self._cases:
            ...
            __cos, __sin = mlable.schedules.cosine_rates(__rand, start_rate=__case['args']['start_rate'], end_rate=__case['args']['end_rate'])

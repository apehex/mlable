...
class TransformerBlockTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._cases = [
            {
                'inputs': tf.ones((2, 16, 16, 8), dtype=tf.float16),
                'args': {'channel_dim': 8, 'group_dim': 2, 'head_dim': 1, 'layer_num': 1, 'dropout_rate': 0.1, 'epsilon_rate': 1e-6, 'use_causal_mask': True,},
                'outputs': {'shape': (2, 16, 16, 8),},},
            {
                'inputs': tf.convert_to_tensor([16 * [16 * [8 * [__i]]] for __i in range(4)], dtype=tf.float32),
                'args': {'channel_dim': 8, 'group_dim': 4, 'head_dim': 2, 'layer_num': 2, 'dropout_rate': 0.0, 'epsilon_rate': 1e-6, 'use_causal_mask': False,},

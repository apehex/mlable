...
class MaskingTest(tf.test.TestCase):
    ...
    def test_reduce_shapes(self):
        ...
        __m_0_any = mlable.masking.reduce_any_mask(mask=self._ascii, axis=0, keepdims=False)
        __m_0_all = mlable.masking.reduce_all_mask(mask=self._ascii, axis=0, keepdims=False)
        __m_1_any = mlable.masking.reduce_any_mask(mask=self._ascii, axis=-1, keepdims=False)
        __m_1_all = mlable.masking.reduce_all_mask(mask=self._ascii, axis=-1, keepdims=False)
        ...
        # reduced on all axes
        __m_any = mlable.masking.reduce_any_mask(mask=self._ascii, axis=None, keepdims=False)
        __m_all = mlable.masking.reduce_all_mask(mask=self._ascii, axis=None, keepdims=False)
        self.assertEqual(list(__m_any.shape), [])
        self.assertEqual(list(__m_all.shape), [])
        # same shape as input
        __m_0_any_keep = mlable.masking.reduce_any_mask(mask=self._ascii, axis=0, keepdims=True)
        __m_0_all_keep = mlable.masking.reduce_all_mask(mask=self._ascii, axis=0, keepdims=True)
        __m_1_any_keep = mlable.masking.reduce_any_mask(mask=self._ascii, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.masking.reduce_all_mask(mask=self._ascii, axis=-1, keepdims=True)
        __m_any_keep = mlable.masking.reduce_any_mask(mask=self._ascii, axis=None, keepdims=True)
        __m_all_keep = mlable.masking.reduce_all_mask(mask=self._ascii, axis=None, keepdims=True)
    ...
    def test_group_shapes(self):
        # reduced along a single axis
        __m_0_any = mlable.masking.group_any_mask(mask=self._ascii, group=2, axis=0, keepdims=False)
        __m_0_all = mlable.masking.group_all_mask(mask=self._ascii, group=2, axis=0, keepdims=False)
        __m_1_any = mlable.masking.group_any_mask(mask=self._ascii, group=4, axis=-1, keepdims=False)
        __m_1_all = mlable.masking.group_all_mask(mask=self._ascii, group=4, axis=-1, keepdims=False)
        ...
        # same shape as input
        __m_0_any_keep = mlable.masking.group_any_mask(mask=self._ascii, group=2, axis=0, keepdims=True)
        __m_0_all_keep = mlable.masking.group_all_mask(mask=self._ascii, group=2, axis=0, keepdims=True)
        __m_1_any_keep = mlable.masking.group_any_mask(mask=self._ascii, group=4, axis=-1, keepdims=True)
        __m_1_all_keep = mlable.masking.group_all_mask(mask=self._ascii, group=4, axis=-1, keepdims=True)
        self.assertEqual(list(__m_0_any_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_0_all_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_1_any_keep.shape), list(self._ascii.shape))
        self.assertEqual(list(__m_1_all_keep.shape), list(self._ascii.shape))
